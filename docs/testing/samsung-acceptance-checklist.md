...
| Full Android instrumentation suite | Pass | All 22 tests passed together on SM-S928U / Android 16, including branding identity, Room migration, Library, navigation, playback controls, backup behavior, additive multiple-folder sources, separate clickable track cards, artist/genre/playlist drill-in, direct and bulk playlist additions, dismissible scan results, and a dedicated scan screen that visibly identifies Music, Please! while preserving its mode title, progress, and exit control. |
...
| Multiple user-selected folder flow | Automated pass | SAF read-only grant tests prove distinct folders append without replacing earlier sources and repeated URIs are deduplicated; on-device UI tests show both folders, “Add another folder,” and no specific-file action. Interactive system-folder choice remains a user acceptance step. |
| Whole-device denial preserves scoped sources | Automated pass | Permission-flow test. |
| First source starts dedicated scan; later sources scan quietly | Automated pass | Source-addition policy tests cover user-selected folders and optional whole-device entry points; personal-library throughput remains a user acceptance step. |
